...
(() => {
      ...
      var DataLoader3 = /* @__PURE__ */ function() {
        ...
        _proto.loadMany = function loadMany(keys) {
          ...
          for (var i4 = 0; i4 < keys.length; i4++) {
            loadPromises.push(this.load(keys[i4])["catch"](function(error) {
              return error;
            }));
          }
      ...
      function dispatchBatch(loader, batch) {
        ...
        batchPromise.then(function(values) {
          ...
          for (var i4 = 0; i4 < batch.callbacks.length; i4++) {
            var _value = values[i4];
            if (_value instanceof Error) {
              batch.callbacks[i4].reject(_value);
            } else {
              batch.callbacks[i4].resolve(_value);
            }
          }
  ...
  var hexes = Array.from({ length: 256 }, (v, i4) => i4.toString(16).padStart(2, "0"));
  ...
  function chain(...args) {
    ...
    const decode4 = args.reduce((acc, i4) => acc ? wrap(acc, i4.decode) : i4.decode, void 0);
  ...
  var hexes2 = /* @__PURE__ */ Array.from({ length: 256 }, (_, i4) => i4.toString(16).padStart(2, "0"));
  ...
  function weierstrassPoints(opts) {
    ...
    class Point2 {
      ...
      static normalizeZ(points) {
        ...
        return points.map((p, i4) => p.toAffine(toInv[i4])).map(Point2.fromAffine);
      }
      static fromHex(hex2) {
      ...
      wNAF(n) {
        return wnaf.wNAFCached(this, pointPrecomputes, n, (comp) => {
          ...
          return comp.map((p, i4) => p.toAffine(toInv[i4])).map(Point2.fromAffine);
        });
      }
      multiplyUnsafe(n) {
        const I = Point2.ZERO;
  ...
  var u8to16 = (a, i4) => a[i4++] & 255 | (a[i4++] & 255) << 8;
  var Poly1305 = class {
    constructor(key) {
      this.blockLen = 16;
      this.outputLen = 16;
  ...
  function makeListFetcher(kind, hardcodedRelays, process2, defaultTo) {
    ...
    const dataloader = new import_dataloader.default((requests) => new Promise(async (resolve) => {
      ...
      let results = await getMany(requests.map((r) => r.target), store).then((results2) => results2.map((res, i4) => {
  ...
  var MemoryHints = class {
    ...
    async topN(pubkey, n) {
      ...
      if (rfpk) {
        ...
        for (let i4 = 0; i4 < n && i4 < rfpk.length; i4++) {
          urls.push(this.relayBySerial[rfpk[i4].serial]);
        }
      }
      return urls;
    }
    printScores() {
      ...
      for (let pubkey in this.orderedRelaysByPubKey) {
        ...
        for (let i4 = 0; i4 < rfpk.entries.length; i4++) {
          const re = rfpk[i4];
          console.log(`  ${i4.toString().padStart(3)} :: ${this.relayBySerial[re.serial].padEnd(30)} (${re.serial}) ::> ${re.sum().toString().padStart(12)}`);
        }
      }
    }
  };
  var RelayEntry = class {
    ...
    sum() {
      ...
      let sum = 0;
      for (let i4 = 0; i4 < this.timestamps.length; i4++) {
        if (this.timestamps[i4] === 0)
          continue;
        const value = hintBasePoints[i4] * 1e10 / Math.pow(Math.max(now2 - this.timestamps[i4], 1), 1.3);
        sum += value;
      }
      return sum;
    }
  ...
  var metadataLoader = new import_dataloader2.default(async (requests) => new Promise(async (resolve) => {
    const toFetch = [];
    let now2 = Math.round(Date.now() / 1e3);
    let results = await getMany(requests.map((r) => r.pubkey), customStore).then((results2) => results2.map((res, i4) => {
      const req = requests[i4];
      if (!res) {
        toFetch.push(req);
      ...
      let h = pool.subscribeManyMap(filtersByRelay, {
        ...
        onevent(evt) {
          for (let i4 = 0; i4 < requests.length; i4++) {
            if (requests[i4].pubkey === evt.pubkey) {
              const nu = results[i4];
  ...
  var NostrUserSearch = class extends HTMLElement {
    ...
    limit = 10;
    value;
    relays = ["wss://nostr.wine", "wss://search.nos.today", "wss://relay.nostr.band"].map(normalizeURL3);
    constructor() {
      super();
      this.attachShadow({ mode: "open" });
      const { shadowRoot } = this;
    ...
    attributeChangedCallback(name, _old, value) {
      if (name === "relays") {
        this.relays = splitComma(value).map(normalizeURL3);
      } else if (name === "limit") {
    ...
    search = debounce(async (q) => {
      for (let results of this.queryPart("results")) {
        ...
        for (let i4 = 0; i4 < results.children.length; i4++) {
          if (results.children[i4].tagName === "TEMPLATE")
            continue;
          results.removeChild(results.children[i4]);
        }
        results.style.display = "block";
      }
      pool2.subscribeManyEose(this.relays, [{ search: q, limit: this.limit, kinds: [0] }], {
        onevent: (evt) => {
    ...
    *queryPart(name) {
      ...
      for (let i4 = 0; i4 < slotted.length; i4++) {
        yield slotted[i4];
      }
      let templated = this.root.querySelectorAll(`[part="${name}"]`);
      for (let i4 = 0; i4 < templated.length; i4++) {
        yield templated[i4];
      }
    }
  };
  ...
  var NostrEventJson = class extends HTMLElement {
    ...
    set = debounce(async () => {
      ...
      if (input) {
        ...
        relays = relays.map(normalizeURL3);
        if (author)
  ...
  var NostrLivestream = class extends HTMLElement {
    ...
    connectedCallback() {
      setTimeout(() => {
        ...
        <video part="video" controls autoplay style="display: none;">
  ...
  __publicField(NostrLivestream, "observedAttributes", ["ref"]);
  ...
  var JS3 = class {
    ...
    finalizeEvent(t, secretKey) {
      ...
      event.sig = bytesToHex(schnorr.sign(getEventHash3(event), secretKey));
  ...
  function parse4(event) {
    ...
    let maybeParent;
    let maybeRoot;
    for (let i4 = event.tags.length - 1; i4 >= 0; i4--) {
    ...
    switch (prefix) {
      case "nprofile": {
        ...
        return {
          ...
          data: {
            pubkey: bytesToHex(tlv[0][0]),
            relays: tlv[1] ? tlv[1].map((d) => utf8Decoder6.decode(d)) : []
          }
        };
      }
      case "nevent": {
      ...
      case "naddr": {
        ...
        return {
          type: "naddr",
          data: {
            identifier: utf8Decoder6.decode(tlv[0][0]),
            pubkey: bytesToHex(tlv[2][0]),
            kind: parseInt(bytesToHex(tlv[3][0]), 16),
            relays: tlv[1] ? tlv[1].map((d) => utf8Decoder6.decode(d)) : []
          }
  ...
  var noCharacter2 = /\W/m;
  var noURLCharacter2 = /\W |\W$|$|,| /m;
  function* parse5(content) {
    ...
    while (index < max) {
      ...
      if (content.substring(u - 5, u) === "nostr") {
        const m = content.substring(u + 60).match(noCharacter2);
        const end = m ? u + 60 + m.index : max;
        try {
          let pointer;
          let { data, type } = decode3(content.substring(u + 1, end));
          switch (type) {
            case "npub":
              pointer = { pubkey: data };
              break;
        ...
        const m = content.substring(u + 4).match(noURLCharacter2);
        const end = m ? u + 4 + m.index : max;
        const prefixLen = content[u - 1] === "s" ? 5 : 4;
        try {
          let url = new URL(content.substring(u - prefixLen, end));
  ...
  function renderText(wrapper, content) {
    for (let block of parse5(content)) {
      switch (block.type) {
        case "text":
    ...
    set = debounce(async () => {
      ...
      if (!evt) {
        ...
        if (ref) {
          ...
          let d = data;
          let relays = (d.relays || []).concat(splitComma(this.getAttribute("relays")).map(normalizeURL3));
          let filter = { ids: [d.id] };
  ...
  var NostrFollow = class extends HTMLElement {
    ...
    *queryPart(name) {
      ...
      for (let i4 = 0; i4 < slotted2.length; i4++) {
        yield slotted2[i4];
      }
    }
    async handleClick() {
