...
/* // Simplified subscription management - single subscription at a time
function startSubscription(filter, relays, options = {}) {
  // Ensure relays is an array
  if (!Array.isArray(relays)) {
    console.error("Relays must be an array:", relays);
    return null;
  }

  if (relays.length === 0) {
    console.error("No relays provided");
    return null;
  }

  // Close existing subscription if it exists
  if (app.currentSubscription) {
    stopSubscription();
  }

  // Clear events for new subscription
  app.subscriptionEvents = [];
  ...
  const subscription = {
    ...
    relayStates: new Map(),
    isActive: true,
    pool: new window.NostrTools.SimplePool(), // Create pool per subscription
  ...
  // Store as current subscription
  app.currentSubscription = subscription;
...
function _startNostrSubscription(subscription) {
  try {
    const sub = subscription.pool.subscribeMany(
      subscription.relays,
      [subscription.filter],
      {
        onevent: (event, relayUrl) => {
          _handleEvent(event, relayUrl);
        },
        oneose: (relayUrl) => {
          _handleEose(relayUrl);
        },
        onclose: (relayUrl, reason) => {
          _handleClose(relayUrl, reason);
        },
        onerror: (relayUrl, error) => {
          _handleError(relayUrl, error);
        },
      }
    );
...
function _handleEvent(event, relayUrl) {
  const subscription = app.currentSubscription;
  if (!subscription || !subscription.isActive) return;
  ...
  // Deduplication and relay tracking
  const existingEventIndex = app.subscriptionEvents.findIndex(
    (e) => e.event.id === event.id
  );

  if (existingEventIndex !== -1) {
    // Event exists, add relay if not already tracked
    const existingEvent = app.subscriptionEvents[existingEventIndex];
    if (!existingEvent.relays.includes(relayUrl)) {
      existingEvent.relays.push(relayUrl);
    }
  } else {
    ...
    app.subscriptionEvents.push(eventData);

    // Sort by created_at (newest first)
    app.subscriptionEvents.sort((a, b) => b.event.created_at - a.event.created_at);

    // Apply limit if specified
    if (subscription.options.limit && app.subscriptionEvents.length > subscription.options.limit) {
      app.subscriptionEvents.splice(subscription.options.limit);
    }

    // Call custom event handler for new events only
    if (subscription.onEvent) {
      subscription.onEvent(eventData, subscription);
    }
  }
...
function _handleClose(relayUrl, reason) {
  const subscription = app.currentSubscription;
  if (!subscription) return;
...
function _handleError(relayUrl, error) {
  const subscription = app.currentSubscription;
  if (!subscription) return;
...
function stopSubscription() {
  ...
  if (subscription) {
    ...
    app.currentSubscription = null;
    app.subscriptionEvents = [];
  }
}

function getSubscriptionEvents() {
  return app.subscriptionEvents || [];
}

function clearSubscriptionEvents() {
  app.subscriptionEvents = [];
}
      ...
      <div class="filter-section">
        ...
        <div id="filter-form-container" class="collapsible">
          <form id="filter-form" class="filter-form">
            ...
            <div class="form-field">
              <label>Relays (one per line)</label>
              <textarea id="relays" rows="3">wss://nos.lol
wss://nostr.mom</textarea>
      ...
      <div class="subscription-section">
        <div class="section-header">
          ...
          <button id="stop-subscription" class="btn btn-secondary" style="display:none">Stop</button>
        </div>
        <div id="subscription-status" class="subscription-status">
          <div class="no-subscription">No active subscription</div>
        </div>
      </div>
...
function setupSubscriptionManager() {
  ...
  const elements = {
    ...
    formContainer: document.getElementById("filter-form-container"),
    startBtn: document.getElementById("start-subscription"),
    stopBtn: document.getElementById("stop-subscription"),
  ...
  const updateInterval = setInterval(updateSubscriptionDisplay, 2000);
  ...
  function getRelays() {
    const relayText = document.getElementById("relays").value.trim();
    if (!relayText) return [];
  ...
  function startNewSubscription() {
    ...
    startSubscription(filter, relays, {
      limit: filter.limit || 50,
      onEvent: (eventData) => {
        if (!isEventsPaused) {
          displayEvent(eventData);
  ...
  function displayEvent(eventData) {
    const { event, relays, receivedAt } = eventData;
    ...
    // Show all relays that received this event
    const relayBadges = relays.map(relay => 
      `<span class="relay-badge" title="${relay}">${relay.split('//')[1]?.split('.')[0] || relay}</span>`
    ).join(' ');
      ...
      eventEl.innerHTML = `
        <div class="event-header">
          <div class="event-meta">
            <span class="kind-badge">Kind ${event.kind}</span>
            <span class="time-badge">${eventTime}</span>
            <div class="relay-badges">${relayBadges}</div>
          </div>
        </div>
        <pre class="event-json">${JSON.stringify(event, null, 2)}</pre>
      `;
    ...
    elements.eventsContainer.insertBefore(eventEl, elements.eventsContainer.firstChild);
  ...
  function updateSubscriptionDisplay() {
    ...
    if (!status) {
      elements.subscriptionStatus.innerHTML = '<div class="no-subscription">No active subscription</div>';
    ...
    const relayStatesHTML = status.relayStates.map(state => {
      const statusClass = state.status.toLowerCase();
      const statusText = state.status === "connecting" && status.duration > 5000 ? "timeout" : state.status;
      ...
      return `
        <div class="relay-state relay-${statusClass}">
          ...
          <span class="relay-events">${state.eventCount} events</span>
          ${state.eose ? '<span class="relay-eose">EOSE</span>' : ""}
          ${state.error ? `<span class="relay-error" title="${state.error}">⚠️</span>` : ""}
        </div>
      `;
    }).join("");
    ...
    elements.subscriptionStatus.innerHTML = `
      <div class="subscription-info">
        <div class="sub-stats">
          ...
          <span>${status.connectedRelays}/${status.totalRelays} relays</span>
          <span>${formatDuration(status.duration / 1000)}</span>
        </div>
        <div class="relay-states">${relayStatesHTML}</div>
      </div>
    `;
  }
  ...
  function refreshEventsDisplay() {
    ...
    // Display events from current subscription
    const events = getSubscriptionEvents();
    events.slice(0, eventDisplayLimit).forEach((eventData) => {
      displayEvent(eventData);
    });
  }
  ...
  function clearEvents() {
    elements.eventsContainer.innerHTML = '<div class="no-events">No events yet</div>';
  }
...
function startChatSubscription(channelId) {
  ...
  return startSubscription(filter, relays, {
    onEvent: (eventData) => {
      // eventData.relays contains all relays that sent this event
      renderChatMessage(eventData.event, eventData.relays);
    }
  });
} */