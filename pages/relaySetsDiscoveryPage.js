...
function createRelayItemHtml(url) {
  ...
  const uniqueIconId = `icon-feed-${crypto.randomUUID().slice(0, 8)}`;
  
  const itemHtml = `
    <div class="relay-item" data-relay="${escapeHtml(url)}" data-icon-id="${uniqueIconId}">
        ...
        <div class="relay-main-info">
          <div class="relay-identity">
            ...
            <div class="relay-text-info">
              <h3 class="relay-name">${escapeHtml(domain)}</h3>
              <div class="relay-url">${escapeHtml(url)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="relay-actions">
        ...
        <button class="btn btn-primary relay-add-btn" data-url="${escapeHtml(url)}">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  setTimeout(() => loadRelayIcon(url, uniqueIconId), 0);
